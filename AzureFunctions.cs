        ...
        [HttpTrigger(AuthorizationLevel.Anonymous, "get", "post")] HttpRequest req)
        {
            ...
            using (var clientContext = new AuthenticationManager().GetACSAppOnlyContext(siteURL, clientID, clientSecretID))
            {
                using (PnPContext pnpCoreContext = PnPCoreSdk.Instance.GetPnPContext(clientContext))
                {
                    ...
                    var lastChangeToken = await Services.GetLatestChangeTokenAsync(resourceId, AzureBlobStorageConnectionString, BlobContainerCSV);
                    ...
                    if (changes.Any())
                    {
                        Services.SaveLatestChangeTokenAsync(changes.Last().ChangeToken, resourceId, AzureBlobStorageConnectionString, BlobContainerCSV);
                    }
                    ...
                    if (addChangesList.Count > 0)
                    {
                                        ...
                                        if (item.FileSystemObjectType == PnP.Core.Model.SharePoint.FileSystemObjectType.File)
                                        {
                                            ...
                                            if (serverRelativeURL.ToString().ToUpperInvariant().Contains("CSV"))
                                            {
                                                ...
                                                foreach (var record in records)
                                                {
                                                    ...
                                                    {
                                                        emetteursListProcessed = Services.FormatEmails(emetteursList, prefix);
                                                    }
                                                    if (!string.IsNullOrEmpty(verificateursList))
                                                    {
                                                        verificateursListProcessed = Services.FormatEmails(verificateursList, prefix);
                                                    }
                                                    if (!string.IsNullOrEmpty(approbateursList))
                                                    {
                                                        approbateursListProcessed = Services.FormatEmails(approbateursList, prefix);
                                                    }
                                                    if (!string.IsNullOrEmpty(chef))
                                                    {
                                                        chefProcessed = Services.FormatEmails(chef, prefix);
                                                    }
                                                    if (!string.IsNullOrEmpty(secretaire))
                                                    {
                                                        secretaireProcessed = Services.FormatEmails(secretaire, prefix);
                                                    }
                                                    if (!string.IsNullOrEmpty(intervenantPAOProcessed))
                                                    {
                                                        intervenantPAOProcessed = Services.FormatEmails(intervenantPAOProcessed, prefix);
                                                    }
                                                    if (!string.IsNullOrEmpty(intervenantTTXProcessed))
                                                    {
                                                        intervenantTTXProcessed = Services.FormatEmails(intervenantTTXProcessed, prefix);
                                                    }
                                                    ...
                                                    {
                                                        clientIDs = Services.GetItemIdsBySiteTitles(clientContext, "Clients", client, "Nom_x0020_Client");
                                                    }

                                                    if (!string.IsNullOrEmpty(projet))
                                                    {
                                                        projetID = Services.GetItemIdsBySiteTitles(clientContext, "Codes Projets", projet, "Libell_x00e9_");
                                                    }


                                                    if (!string.IsNullOrEmpty(DossierPays))
                                                    {
                                                        paysID = Services.GetItemIdsBySiteTitles(clientContext, "Codes Pays", DossierPays, "Libell_x00e9_");
                                                    }
                                                    if (!string.IsNullOrEmpty(sites))
                                                    {
                                                        sitesID = Services.GetItemIdsBySiteTitles(clientContext, "Liste des Sites", sites, "Site");
                                                    }
                                                    if (!string.IsNullOrEmpty(motsCles))
                                                    {
                                                        motsClesIDs = Services.GetItemIdsBySiteTitles(clientContext, "Mots-Clés", motsCles, "Title");
                                                    }
                                                    if (!string.IsNullOrEmpty(societe))
                                                    {
                                                        societeIDs = Services.GetItemIdsBySiteTitles(clientContext, "Société", societe, "Title");
                                                    }
                                                    if (!string.IsNullOrEmpty(departement))
                                                    {
                                                        departementIDs = Services.GetItemIdsBySiteTitles(clientContext, "Departements", departement, "Libell_x00e9_");
                                                    }
                                                    ...
                                                    Dictionary<string, object> listItem = new Dictionary<string, object>()
                        {
                            ...
                            {"creatorEmail","powerauto_p2022121914@vinci-construction.com" },
                            { "SiteURL", ""+SiteURL },
                            { "ListName", ""+Type },
                            { "JSON", "\""+JSONTemplate+"\"" },
                            { "Statut_x0020_webhook", ""+"En attente de création"+"" },
{"ModeCreation", "CSV" }
        ...
        [HttpTrigger(AuthorizationLevel.Anonymous, "get", "post")] HttpRequest req)
        {
            ...
            using (var clientContext = new AuthenticationManager().GetACSAppOnlyContext(siteURL, clientID, clientSecretID))
            {
                using (PnPContext pnpCoreContext = PnPCoreSdk.Instance.GetPnPContext(clientContext))
                {
                    ...
                    //For test purposes, use this for faster 
                    var result =  await Services.DossierMaitreCreation(clientContext, pnpCoreContext, 269, targetList, _logger);
                    results.Add(result);
                    ...
                    var lastChangeToken = await Services.GetLatestChangeTokenAsync(resourceId, AzureBlobStorageConnectionString, BlobContainerList);
                    ...
                    if (changes.Any())
                    {
                      Services.SaveLatestChangeTokenAsync(changes.Last().ChangeToken, resourceId, AzureBlobStorageConnectionString, BlobContainerList);
                    }
                    var addChangesList = changes.Where(change => change.ChangeType == PnP.Core.Model.SharePoint.ChangeType.Add).ToList();
                    if (addChangesList.Count > 0)
                    {
                        foreach (var change in addChangesList)
                        {
                            if (change is IChangeItem changeItem)
                            {
                                if (changeItem.IsPropertyAvailable<IChangeItem>(i => i.ItemId))
                                {
                                 //var result = await Services.DossierMaitreCreation(clientContext, pnpCoreContext, changeItem.ItemId, targetList, _logger);
                                 //results.Add(result);
